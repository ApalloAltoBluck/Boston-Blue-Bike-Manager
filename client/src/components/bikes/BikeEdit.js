...
function BikeEdit() {
  ...
  React.useEffect(() => {
    findBikeById(params.id).then((res) => setBike(res[0]));
    findAllStations().then((response) =>
      setAvailableStation(response.message.map(({ station_id }) => station_id))
    );
    findAllUsers().then((response) =>
      setAvailableUser(response.message.map(({ userID }) => userID))
    );
  }, []);
  ...
  return (
    <header className="App-header row">
      <div className="col">
        ...
        {!bike ? (
          ...
          <>
            ...
            >
              {availableUser &&
                availableUser.map((user) => (
                  <option
                    selected={user == bike.user ? "selected" : ""}
                    value={user}
                  >
                    {user}
                  </option>
            ...
            >
              {availableStation &&
                availableStation.map((station) => (
                  <option
                    selected={station == bike.station_id ? "selected" : ""}
                    value={station}
                  >
                    {station}
                  </option>
      ...
      <div className=" col float-right">
        {bike && bike.station_id && (
          <>
            {" "}
            <h1>Station associated with bike</h1>
            <Link to={`/stations/edit/${bike.station_id}`}>
        ...
        {bike && bike.user && (
          <>
            {" "}
            <h1>User associated with bike</h1>
